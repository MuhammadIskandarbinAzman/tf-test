...
public class EntryResource {
    ...
    public ResponseEntity<EntryDTO> createEntry(@Valid @RequestBody EntryDTO entryDTO) throws URISyntaxException {
      ...
      const stringArray = blog.name.toUpperCase().split(' ') <== content
      let error = false
      stringArray.forEach((item) => {
        if (item === 'SAD' || item === 'ANGRY') {
          error = true
        } 
      })
      if (error) {
    ...
    } else if (!blog.positive) {
      // i like apple => ['I', 'LIKE', 'APPLE']
      const stringArray = blog.name.toUpperCase().split(' ')
      let error = false
      stringArray.forEach((item) => {
        if (item === 'HAHA' || item === 'LIKE') {
          error = true
        } 
      })
    ...
    public ResponseEntity<Void> cleanBlog() {
        log.debug("REST request to clean Blog : {}");
        // 1st step: get the keyword
        // 2nd step: blogService.findAll(); => ['']
        // 3rd step: filtering and deleteing
        const keyword = ? from somewhere
        const testingarray =  [{
          id: id2,
          title: 'asd2',
          content: 'qwe2',
        ...
        testingarray.forEach((item) => {
          if (item.title.touppercase().includes(keyword) || item.content.touppercase().includes(keyword)) {
    ...
    public ResponseEntity<Void> cleanBlog(@PathVariable Long id) {
        log.debug("REST request to clean Blog : {}");
        // 1st step: blogService.findOne(id); => {blog}
        // 2nd step: filtering and deleteing
        const keyword = ? from somewhere
        const testingblog = {
          id: id2,
          title: 'asd2',
        ...
        if (testingblog.title.touppercase().includes(keyword) || testingblog.content.touppercase().includes(keyword)) {
          blogService.delete(item.id);
        }
        // blogService.delete(id);
        // return ResponseEntity.noContent().headers(HeaderUtil.createEntityDeletionAlert(applicationName, false, ENTITY_NAME, id.toString())).build();
